...
export default {
  ...
  methods:{
    init(){ // 初始化表格数据
       this.$htp.get('programConfigName/list').then((res)=>{
        if(res.code===200){
    ...
    handleDelete(index, row){ // 删除
       this.$htp.delete(`programConfigName/${row.name}`).then((res)=>{
        if(res.code===200){
    ...
    save(){ // 保存数据
    if(this.status==0){
      this.$htp.post('programConfigName/save', this.dialogData).then((res)=>{
        if(res.code===200){
         ...
         this.$htp.put('programConfigName/save', this.dialogData).then((res)=>{
        if(res.code===200){
    ...
    getDetail(name){ // 获取详情
       this.$htp.get(`programConfigName/${name}`).then((res)=>{
        if(res.code===200){
          this.centerDialogVisible = true;
