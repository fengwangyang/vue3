...
export default {
  ...
  methods:{
    ...
    handleDelete(index, row){ // 删除
       this.$htp.delete(`connectioninfo/${row.name}`).then((res)=>{
        if(res.code===200){
    ...
    save(){ // 保存数据
    if(this.status==0){
      this.$htp.post('connectioninfo/save', this.dialogData).then((res)=>{
        if(res.code===200){
         ...
         this.$htp.put('connectioninfo/save', this.dialogData).then((res)=>{
        if(res.code===200){
    ...
    getDetail(name){ // 获取详情
       this.$htp.get(`connectioninfo/${name}`).then((res)=>{
        if(res.code===200){
          this.centerDialogVisible = true;
